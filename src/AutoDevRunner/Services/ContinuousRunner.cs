...
public class ContinuousRunner
{
    ...
    public ContinuousRunner(
        DueProjectsRunner due, ProviderRegistry providers, ProviderAvailability availability,
        CodexUsageReader codexUsage, IOptions<AutoDevOptions> opt, ILogger<ContinuousRunner> log)
    {
        _due = due; _providers = providers; _availability = availability;
    ...
    {
        _log.LogInformation(
            "Continuous mode: looping runs until providers reach {Max}% usage (cap {Cap} runs, {Delay}s between runs).",
            _opt.MaxUsagePercent, _opt.MaxRunsPerSession, _opt.DelayBetweenRunsSeconds);
