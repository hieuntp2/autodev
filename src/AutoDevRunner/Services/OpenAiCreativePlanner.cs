...
public class OpenAiCreativePlanner
{
    ...
    {
        // Whether to call at all is decided by PlannerCallPolicy (global default
        // + per-project PlannerProvider override) — not re-checked here, so a
        // project that explicitly selects OpenAI works even when the global
        // Planner:Enabled switch is off.
    ...
    private JsonObject BuildRequestBody(Project project, string brief, ProjectGoal? goal)
    {
        ...
        {
            ["model"] = _opt.Model,
            ["input"] = PlannerPromptBuilder.BuildInstructions(project, brief, goal)
        };
